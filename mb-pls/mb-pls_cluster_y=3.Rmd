...
X.2 <- res.mbpls2$lX[, 1:3]  # Ensure this is a numeric matrix/data.frame
...
plot_func <- function(cluster){
  grid$prob_cluster1 <- grid_probs[,cluster]
    
  # Données pour les points
  plot_data <- data.frame(
    X.2[,1:2], 
    y_malade = factor(data.clean$y, levels = c("ENG_malade", "PS_malade", "Sain"))
  )
...
colnames(res.mb) <- c("Ax1", "Ax2", "Ax3", "clust")
for (j in 1:3){
  for (i in 1:3) {
...
# Load latent components
X.4 <- res.mbpls2$lX[, 1:4]  # Ensure this is a numeric matrix/data.frame
# Fit models and select best using BIC
results <- data.frame(
  Model = character(),
  G = numeric(),
...
# Get all model names
model_names <- c("EII", "VII", "EEI", "VEI", "EVI", "VVI", "EEE", "EVE", "VEV", "VVV", "VEE")

# Loop through models and cluster numbers
for(model in model_names) {
  for(g in 1:10) {
    # Fit model
    fit <- Mclust(X.4, G = g, modelNames = model)
    
    # Store results if model converged
...
# Create the plot
ggplot(results, aes(x = G, y = LogLik, color = Model)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  scale_x_continuous(breaks = 1:10) +
  labs(title = "Log-Likelihood by Model and Number of Clusters",
       x = "Number of Clusters (G)",
       y = "Log-Likelihood") +
  theme_minimal() +
  theme(legend.position = "bottom")

# Optional: Facet by model for better readability
ggplot(results, aes(x = G, y = LogLik)) +
...
ggplot(grid, aes_string(x = original_names[1], y = original_names[2])) +
  ...
  labs(x = "Axe 1 mb-pls", y = "Axe 2 mb-pls") +
  theme_minimal()
grid$prob_cluster1 <- grid_probs[,4]
...
ggplot(grid, aes_string(x = original_names[1], y = original_names[2])) +
  ...
  labs(x = "Axe 1 mb-pls", y = "Axe 2 mb-pls") +
  theme_minimal()
grid$prob_cluster1 <- grid_probs[,5]
...
Clusters pas trés interpretables suivant y.
