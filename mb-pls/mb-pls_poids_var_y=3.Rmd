...
On s'arrête ici.
## Doit-on refaire analyses de classification avec nouveau modèle de la mb-pls
# Matrice de confusion pour le modèle final

```{r}
coef_obs1 <- res.mbpls2$XYcoef[[1]][,1:4]
coef_obs2 <- res.mbpls2$XYcoef[[2]][,1:4]
coef_obs3 <- res.mbpls2$XYcoef[[3]][,1:4]
...
class.pred <- apply(Y.pred, 1, function(x) {((which.max(x) - 1) %/% 4) + 1}) 
# on pourra essayer de faire la moyenne par axe (pondérée par explicaion de Y par exemple et appliquer ensuite la fonction en ayant ainsi que trois colonnes, à demander)
# Intérêt de comparer es matrices de confusion des 3 diff modeles utilisées ici ?
...
roc1 <- roc(true_binary1, as.numeric(class.pred == "Sain"), plot = TRUE, col = "red", main = "Courbes ROC Multiclasse")
