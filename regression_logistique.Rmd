...
# on commence par travailler avec donnees_mean
...
vars_explicatives <- names(donnees_mean2)[!names(donnees_mean2) %in% "y"]

# Boucle pour vérifier la séparation parfaite
for (var in vars_explicatives) {
  tab <- table(donnees_mean2$y, donnees_mean[[var]])
...
vars_quanti <- names(donnees_mean2)[sapply(donnees_mean2, is.numeric)]
...
for (var in vars_quanti) {
  ...
  model <- vglm(formula, family = multinomial, data = donnees_mean)
  ...
  if (any(abs(coef_val) > 10)) {
    cat("⚠️ Ale/rte : Problème de séparation possible ! Coefficients élevés pour", var, "\n")
  }
...
# Vérification avec les p-values
...
# Vérification de la non-corrélation résidus-prédictions
residus <- residuals(modele_reduit, type = "pearson")
predictions <- predict(modele_reduit, type = "response")

plot(predictions, residus, main = "Résidus vs Prédictions", xlab = "Prédictions", ylab = "Résidus")

# Homoscédasticité : Test de Breusch-Pagan
bptest(modele_reduit)

# Autocorrélation temporelle : Test de Durbin-Watson
dwtest(modele_reduit)
```
